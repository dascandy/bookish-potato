#ifndef FUTURE_H
#define FUTURE_H

#include <functional>
#include <memory>
#include <vector>
#include <utility>

template <typename T>
class shared_state;
template <typename T>
class promise;
template <typename T>
class future;

class executor {
public:
  virtual void queue(const std::function<void()>& f) = 0;
  template <typename T>
  auto async(T func) -> future<decltype(func())>;
};

void run_queued_tasks();
executor& default_executor();

template <typename T>
auto async(T task, executor& ex = default_executor()) -> future<decltype(task())> {
  return ex.async(task);
}

template <typename T>
struct unwrapped_future {
  typedef T type;
};

template <typename T>
struct unwrapped_future<future<T>> {
  typedef T type;
};

template <typename T>
class future {
public:
  T get() const;
  bool is_ready() const;
  template <typename U>
  auto then(U func, executor &exec = default_executor()) -> future<typename unwrapped_future<decltype(func(*(future<T>*)0))>::type>;
//private:
  future(const std::shared_ptr<shared_state<T>> &state);
  future(const future& f)
  : state(f.state)
  {
  }
  ~future() {
  }
  friend class promise<T>;
  std::shared_ptr<shared_state<T>> state;
};

template <typename T>
class shared_state {
public:
  shared_state()
  : storage(nullptr)
  {
  }
  ~shared_state()
  {
    delete storage;
  }
  void set(T &&value) {
    storage = new T(std::move(value));
    value_set = true;
    for (const auto &p : cbs)
      p.first->async(p.second);
    cbs.clear();
  }
  void set(const T &value) {
    storage = new T(value);
    value_set = true;
    for (const auto &p : cbs)
      p.first->async(p.second);
    cbs.clear();
  }
  T get() {
    return *storage;
  }
  void then(const std::function<void()> &cb, executor &exec) {
    if (value_set)
      exec.async(cb);
    else
      cbs.push_back(std::make_pair(&exec, cb));
  }
  std::vector<std::pair<executor*, std::function<void()>>> cbs;
  T *storage;
  bool value_set = false;
  static constexpr const char* name = "shared_state<T>";
};

template <>
struct shared_state<void> {
public:
  void set() {
    value_set = true;
    for (const auto &p : cbs)
      p.first->async(p.second);
    cbs.clear();
  }
  void get() const {
  }
  void then(const std::function<void()> &cb, executor &exec) {
    if (value_set)
      exec.async(cb);
    else
      cbs.push_back(std::make_pair(&exec, cb));
  }
  std::vector<std::pair<executor*, std::function<void()>>> cbs;
  bool value_set = false;
  static constexpr const char* name = "shared_state<void>";
};

template <typename T>
future<T> make_ready_future(T value) {
  std::shared_ptr<shared_state<T>> state = std::make_shared<shared_state<T>>();
  future<T> f(state);
  state->set(std::move(value));
  return f;
}

future<void> make_ready_future();

template <typename T>
class promise;

template <typename T>
T future<T>::get() const {
  return state->get();
}

template <typename T>
bool future<T>::is_ready() const {
  return state->value_set;
}

template <typename T>
future<T>::future(const std::shared_ptr<shared_state<T>> &state)
: state(state)
{
}

template <typename T>
class promise {
public:
  future<T> get_future() { 
    return future<T>(state); 
  }
  void set_value(const T &value) {
    state->set(value);
  }
  std::shared_ptr<shared_state<T>> state = std::make_shared<shared_state<T>>();
  static constexpr const char* name = "promise<T>";
};

template <>
class promise<void> {
public:
  future<void> get_future() { 
    return future<void>(state); 
  }
  void set_value() {
    state->set();
  }
  std::shared_ptr<shared_state<void>> state = std::make_shared<shared_state<void>>();
  static constexpr const char* name = "promise<void>";
};

template <typename T>
struct all {
  bool operator()(const std::vector<future<T>> &futures) {
    for (const auto &f : futures) {
      if (!f.is_ready())
        return false;
    }
    return true;
  }
};

template <typename T>
struct any {
  bool operator()(const std::vector<future<T>> &futures) {
    for (const auto &f : futures) {
      if (f.is_ready())
        return true;
    }
    return false;
  }
};

template <template <typename> class P, typename T>
future<std::vector<future<T>>> when(const std::vector<future<T>> &futures) {
  struct when_holder {
    std::vector<future<T>> futures;
    P<T> pred;
    promise<std::vector<future<T>>> prom;
    bool value_set;
    when_holder(std::vector<future<T>> v) 
    : futures(v)
    {
      value_set = false;
    }
    void updatePromise() 
    {
      if (!value_set && pred(futures))
      {
        prom.set_value(std::move(futures));
        value_set = true;
      }
    }
  };
  std::shared_ptr<when_holder> inst = std::make_shared<when_holder>(futures);
  for (auto &f : inst->futures) {
    f.then([inst](const future<T> &) {
      inst->updatePromise();
    });
  }
  future<std::vector<future<T>>> rv = inst->prom.get_future();
  inst->updatePromise();
  return rv;
}

template <typename T, typename U>
struct then_impl {
  static future<U> impl(future<T> t, const std::function<U(const future<T>&)> &func, executor &exec)
  {
    std::shared_ptr<promise<U>> value = std::make_shared<promise<U>>();
    future<U> rv = value->get_future();
    t.state->then([t, value, func]{
      value->set_value(func(t));
    }, exec);
    return rv;
  }
};

template <typename T>
struct then_impl<T, void> {
  static future<void> impl(future<T> t, const std::function<void(const future<T>&)> &func, executor &exec)
  {
    std::shared_ptr<promise<void>> value = std::make_shared<promise<void>>();
    future<void> rv = value->get_future();
    t.state->then([t, value, func]{ 
      func(t);
      value->set_value(); 
    }, exec);
    return rv;
  }
};

template <typename T, typename U>
struct then_impl<T, future<U> > {
  static future<U> impl(future<T> t, const std::function<future<U>(const future<T>&)> &func, executor &exec)
  {
    std::shared_ptr<promise<U>> value = std::make_shared<promise<U>>();
    future<U> rv = value->get_future();
    t.state->then([t, value, func]{
      func(t).then([value](const future<U>& f) { value->set_value(f.get()); });
    }, exec);
    return rv;
  }
};

template <typename T>
struct then_impl<T, future<void> > {
  static future<void> impl(future<T> t, const std::function<future<void>(const future<T>&)> &func, executor &exec)
  {
    std::shared_ptr<promise<void>> value = std::make_shared<promise<void>>();
    future<void> rv = value->get_future();
    t.state->then([t, value, func]{ 
      func(t).then([value](const future<void>& f) { f.get(); value->set_value(); });
    }, exec);
    return rv;
  }
};

template <typename T>
template <typename U>
auto future<T>::then(U func, executor &exec) -> future<typename unwrapped_future<decltype(func(*(future<T>*)0))>::type> {
  std::function<decltype(func(*(future<T>*)0))(const future<T> &)> f = func;
  return then_impl<T, decltype(func(*(future<T>*)0))>::impl(*this, f, exec);
}

template <typename U>
struct async_impl {
  static future<U> impl(executor *exec, const std::function<U()> &func) {
    std::shared_ptr<promise<U>> value = std::make_shared<promise<U>>();
    future<U> rv = value->get_future();
    exec->queue([value, func]{
      value->set_value(func());
    });
    return rv;
  }
};

template <>
struct async_impl<void> {
  static future<void> impl(executor *exec, const std::function<void()> &func) {
    std::shared_ptr<promise<void>> value = std::make_shared<promise<void>>();
    future<void> rv = value->get_future();
    exec->queue([value, func]{
      func();
      value->set_value();
    });
    return rv;
  }
};

template <typename T>
auto executor::async(T func) -> future<decltype(func())> {
  std::function<decltype(func())()> f = func;
  return async_impl<decltype(func())>::impl(this, f);
}

template <typename T, typename Pred, typename Func>
future<T> async_for(T initial, Pred pred, Func func) {
  if (pred(initial))
    return make_ready_future<T>(initial);

  struct holder {
    promise<T> p;
    void eval(const future<T> &f) {
      T val = f.get();
      if (pred(val)) {
        p.set_value(val);
        delete this;
      } else {
        func(val).then([this](const future<T> &f) {
          this->eval(f);
        });
      }
    }
    holder(T initial, Pred pred, Func func)
    : pred(pred)
    , func(func)
    {
      func(initial).then([this](const future<T> &f) {
        this->eval(f);
      });
    }
    Pred pred;
    Func func;
  };
  return (new holder(initial, pred, func))->p.get_future();
}

#endif


